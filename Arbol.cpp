...
public:
    ...
    {
        raiz = formar_arbol(arreglo, tamano); // funcion recursiva para que solo quede un sector y este sea la raiz
    }
    nodo_arbol *get_raiz()
    {
        return raiz;
...
private:
    nodo_arbol *formar_arbol(nodo_arbol *arreglo, int tamano) // forma el arbol desde las hojas
    {
        ...
        if (tamano > 1) // condicion de salida el arreglo es de largo 1
        {
            ...
            for (int i = 0; i < tamano - 1; i++)
            {
                ...
                if (!arreglo[i].get_usado())
                {
                    ...
                    nuevo_nodo = new nodo_arbol(longitud_menor, latitud_mayor, longitud_mayor, latitud_menor, arreglo[i].get_cantidad_poblacion() + menor_distancia->get_cantidad_poblacion(), arreglo[i].get_numero_casos() + menor_distancia->get_numero_casos());
            ...
            arreglo = formar_arbol(nuevo_arreglo, tamano / 2); //llamada recursiva con el nuevo arreglo
        }
        return arreglo;
    }
    bool es_menor(nodo_arbol* nodo_base, nodo_arbol* nodo_comparacion, double* menor_distancia_encontrada)
    {
        ...
        { /* se busca el punto medio de las latitides y longitudes para dejar solo un valor de latitud y longitud para cada nodo ingresado*/
            double punto_medio_base_latitud = (nodo_base->get_latitud1() - nodo_base->get_latitud2()) / 2;
            double punto_medio_base_longitud = (nodo_base->get_longitud2() - nodo_base->get_longitud1()) / 2;
            double punto_medio_comparacion_latitud = (nodo_comparacion->get_latitud1() - nodo_comparacion->get_latitud2()) / 2;
            double punto_medio_comparacion_longitud = (nodo_comparacion->get_longitud2() - nodo_comparacion->get_longitud1()) / 2;
